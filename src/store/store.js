...
const mutations = {
    ...
    updateUser(state, payload) {
        // Called from firebaseGetUsers for each user when they login or logoff
...
const actions = {
    ...
    handleAuthStateChanged({ commit, dispatch, state }) {
        ...
        if (user) {
            ...
            // console.log("Logged in, in handleAuthStateChanged")
            let userId = firebaseAuth.currentUser.uid
            ...
            firebaseDb.ref('users/' + userId).once('value', snapshot => {
                ...
                // Set online to true this allows immediate feedback
    ...
    firebaseUpdateUser({}, payload) {
        // setup a ref to update the state of the user
    ...
    firebaseGetUsers({ commit }) {
        ...
        firebaseDb.ref('users/').on('child_changed', snapshot => {
            ...
            // this hook will be executed. each user is added to the users array. This because of the on.child_changed....
            let userDetails = snapshot.val()
            // To get the user id we need to go back to the snapshot itself.  Because,
            // snapshot.val only has the fields belonging to it.
            let userId = snapshot.key
    ...
    firebaseStopGettingMessages( { commit }) {
        // Must call this to stop the firebaseGetMessages hook.
        // when a user leaves
    ...
    firebaseSendMessage({}, payload) {
        // console.log("firebaseSendMessage")
